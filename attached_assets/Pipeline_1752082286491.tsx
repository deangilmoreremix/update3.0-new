...
import { useAIResearch, CompanyResearchData } from '../services/aiResearchService';
import { aiEnrichmentService, DealEnrichmentData } from '../services/aiEnrichmentService';
...
const Pipeline: React.FC = () => {
  ...
  const [bulkEnrichingDeals, setBulkEnrichingDeals] = useState(false);
  ...
  const handleAnalyzeDeal = async (deal: Deal) => {
    ...
    try {
      console.log(`🧠 Analyzing deal: ${deal.title}`);
      
      // Use the aiEnrichmentService to analyze the deal
      const enriched = await aiEnrichmentService.enrichDeal({
        title: deal.title,
      ...
      // Update deal with enhanced data
      const updatedDeal = {
        ...deal,
        probability: enriched.probability || Math.min(deal.probability + 15, 95),
        lastEnrichment: {
          confidence: enriched.confidence || Math.min(deal.probability + 20, 90),
          aiProvider: enriched.aiProvider || 'Hybrid AI (GPT-4o + Gemini)',
          timestamp: new Date()
        }
      };
      
      // Add notes about AI analysis if we received insights
      if (insights.length > 0) {
        const aiNotes = `\n\nAI Analysis (${new Date().toLocaleDateString()}):\n- ${insights.join('\n- ')}`;
        updatedDeal.notes = updatedDeal.notes ? updatedDeal.notes + aiNotes : aiNotes;
      }
  ...
  const handleEnrichDeal = async (deal: Deal) => {
    ...
    try {
      console.log(`✨ Enriching deal: ${deal.title}`);
      
      // Get both company information and deal enrichment
      const [companyData, enrichedDeal] = await Promise.all([
        aiResearch.researchCompany(deal.company),
        aiEnrichmentService.enrichDeal({
          title: deal.title,
          company: deal.company,
          contact: deal.contact,
          value: deal.value,
      ...
      // Generate potential needs based on company research
      const potentialNeeds = companyData.potentialNeeds || [];
      const companyDescription = companyData.description || '';
      
      // Update the deal with enhanced data
      const updatedDeal = {
        ...deal,
        probability: enrichedDeal.probability || Math.min(deal.probability + 20, 95),
        notes: deal.notes ? 
          `${deal.notes}\n\nAI Enrichment (${new Date().toLocaleDateString()}):\n${companyDescription}\n\nPotential Needs:\n- ${potentialNeeds.join('\n- ')}` 
          : `AI Enrichment (${new Date().toLocaleDateString()}):\n${companyDescription}\n\nPotential Needs:\n- ${potentialNeeds.join('\n- ')}`,
        lastEnrichment: {
          confidence: enrichedDeal.confidence || 85,
          aiProvider: enrichedDeal.aiProvider || companyData.aiProvider || 'AI Research',
          timestamp: new Date()
        }
      };
  ...
  const handleBulkAnalyzeDeals = async () => {
    const dealsToAnalyze = Object.values(filteredDeals).filter(d => !d.probability || d.probability < 50);
    
    if (dealsToAnalyze.length === 0) {
      alert('All visible deals already have probability scores above 50%. Use "Re-analyze" to update existing scores.');
      return;
    }

    setBulkEnrichingDeals(true);
      ...
      <div className="max-w-full mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 space-y-4 lg:space-y-0">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={handleBulkAnalyzeDeals}
              disabled={bulkEnrichingDeals}
              className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg hover:from-purple-700 hover:to-blue-700 transition-colors disabled:opacity-50"
            >
              <Brain className="w-4 h-4" />
              <span>{bulkEnrichingDeals ? 'Analyzing...' : 'AI Analyze All'}</span>
              <Sparkles className="w-3 h-3 text-yellow-300 ml-1" />
            </button>
          </div>
        </div>
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-purple-900">
                    {bulkEnrichingDeals ? 'Enriching' : 'Analyzing'} deals... ({analysisProgress.current}/{analysisProgress.total})
                  </span>
                </div>
            ...
            {aiResults && !analysisProgress && (
              <div className="flex items-center space-x-3">
                ...
                <span className="font-medium text-green-900">
                  AI Analysis Complete: {aiResults.success} deals analyzed successfully
