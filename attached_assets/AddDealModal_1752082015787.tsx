...
const AddDealModal: React.FC<AddDealModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleAIResearch = async () => {
    ...
    try {
      console.log(`🏢 Starting company research for ${formData.company}`);
      
      // Use both the research service and enrichment service
      const [companyData, enrichedDeal] = await Promise.all([
        aiResearch.researchCompany(formData.company, undefined, researchPriority),
        aiEnrichmentService.enrichDeal({
          title: formData.title || `New Deal for ${formData.company}`,
      ...
      setAiInsights(enrichedDeal.insights || []);
  ...
  const handleContactResearch = async () => {
    if (!contactDetails.name && !formData.contact) {
    ...
    try {
      const contactName = contactDetails.name || formData.contact;
      console.log(`👤 Starting contact research for ${contactName}`);
      
      // Use both the research service and enrichment service
      const [contactData, enrichedContact] = await Promise.all([
        aiResearch.findContactPerson(contactName, formData.company, 'speed'),
        aiEnrichmentService.enrichContact({
          name: contactName,
          company: formData.company
        })
      ]);
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-3">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              {selectedContact.aiScore && (
                                <div className={`h-5 w-5 rounded-full ${
                                  selectedContact.aiScore >= 80 ? 'bg-green-500' : 
                                  selectedContact.aiScore >= 60 ? 'bg-blue-500' :
                                  selectedContact.aiScore >= 40 ? 'bg-yellow-500' : 'bg-red-500'
                                } text-white flex items-center justify-center text-xs shadow-sm`}>
                                  {selectedContact.aiScore}
                                </div>
                              )}
                            </div>
                          ...
                          <div className="flex flex-col">
                            ...
                            <p className="text-sm font-medium capitalize flex items-center">
                              ...
                              {selectedContact.lastEnrichment && (
                                <Sparkles className="w-3 h-3 ml-1.5 text-purple-500" title="AI Enhanced" />
                              )}
                            </p>
                          </div>
                          <div className="flex flex-col">
                            ...
                            <div className="flex items-center">
                              <div className={`w-2 h-2 rounded-full ${
                                selectedContact.interestLevel === 'hot' ? 'bg-red-500' :
                                selectedContact.interestLevel === 'medium' ? 'bg-yellow-500' :
                                selectedContact.interestLevel === 'low' ? 'bg-blue-500' : 'bg-gray-500'
                              } mr-1.5`}></div>
                      ...
                      <div className="relative">
                        ...
                        {contactDetails.name && (
                          <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
                            <Sparkles className="w-4 h-4 text-purple-500 cursor-pointer" title="AI Research Available" />
                          </div>
                        )}
                      </div>
                    </div>
