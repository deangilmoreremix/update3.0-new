...
function processFile(filePath) {
  ...
  try {
    ...
    // Apply all fixes
    const result1 = fixUnusedVariables(fixedContent, problems);
    if (result1.changesMade) {
      fixedContent = result1.content;
      totalChanges = true;
    }

    const result2 = fixExplicitAny(fixedContent, problems);
    if (result2.changesMade) {
      fixedContent = result2.content;
      totalChanges = true;
    }

    const result3 = fixOtherIssues(fixedContent, problems);
    if (result3.changesMade) {
      fixedContent = result3.content;
      totalChanges = true;
    }
...
// Main execution
function main() {
  console.log('🚀 Starting simplified ESLint bulk fix...\n');
  ...
  for (const filePath of tsFiles) {
    const problems = getESLintProblems(filePath);
    const hasHighPriorityIssues = problems.some(p => 
      priorityPatterns.some(pattern => pattern.test(p.ruleId))
    );
  ...
  try {
    ...
    console.log('\n⚠️  Some issues remain, but build still works.');
  }
}
