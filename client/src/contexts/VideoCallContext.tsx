...
export const VideoCallProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Native WebRTC signaling simulation
  const handleSignaling = useCallback(async (offer?: RTCSessionDescriptionInit, answer?: RTCSessionDescriptionInit, candidates?: RTCIceCandidate[]) => {
    console.log('Handling WebRTC signaling');
    
    // For demo purposes, simulate a self-connection after a short delay
    setTimeout(() => {
      if (localStreamRef.current) {
  ...
  const createPeer = useCallback(async (initiator: boolean, stream: MediaStream) => {
    console.log('Creating WebRTC peer connection, initiator:', initiator);
    ...
    if (initiator) {
      const dataChannel = newPeer.createDataChannel('messages', {
        ordered: true
      });
    ...
    // For demo purposes, simulate WebRTC negotiation
    if (initiator) {
      try {
