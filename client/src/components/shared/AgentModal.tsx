...
const AgentModal: React.FC<AgentModalProps> = ({ agentId, data, onClose }) => {
  ...
  const renderJsonOutput = (data: unknown) => {
    ...
    // Handle proposal generator output
    if (data.proposal && data.keyPoints) {
      return (
        <div className="space-y-4">
          <div className="bg-white shadow rounded-lg p-4 border-l-4 border-amber-500">
            <h4 className="font-bold mb-2 text-amber-800">Proposal</h4>
            <p className="text-sm whitespace-pre-wrap">{data.proposal}</p>
          </div>
          
          <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
            <h4 className="font-bold text-amber-800 mb-2">Key Points</h4>
            <ul className="list-disc pl-5 space-y-1 text-sm">
