...
const Navbar = () => {
  ...
  const connectedApps = [
    ...
    { name: 'White-Label Platform', url: 'https://moonlit-tarsier-239e70.netlify.app', icon: Palette, isExternal: false }
  ];
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 p-4">
                  ...
                  <div className={`absolute top-12 right-0 w-64 ${isDark ? 'bg-gray-900/98 border-white/20' : 'bg-white/98 border-gray-200'} backdrop-blur-2xl border rounded-2xl shadow-2xl z-50 overflow-hidden animate-fade-in`}>
                    <div className="p-4">
                      ...
                      <div className="border-t border-gray-200 dark:border-gray-700 pt-3">
                        <button className={`w-full text-left flex items-center space-x-3 p-2 rounded-lg transition-all duration-200 ${isDark ? 'hover:bg-white/5 text-gray-300 hover:text-white' : 'hover:bg-gray-50 text-gray-600 hover:text-gray-900'}`}>
                          <User size={16} />
                        ...
                        <div className="border-t border-gray-200 dark:border-gray-700 mt-2 pt-2">
                          <button className={`w-full text-left flex items-center space-x-3 p-2 rounded-lg transition-all duration-200 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20`}>
                            <LogOut size={16} />
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className={`lg:hidden mt-4 ${isDark ? 'bg-gray-900/98' : 'bg-white/98'} backdrop-blur-2xl border ${isDark ? 'border-white/10' : 'border-gray-200'} rounded-2xl shadow-2xl overflow-hidden animate-slide-down`}>
          <div className="p-4 space-y-2">
              ...
              >
                ...
                {tab.badge && (
                  <div className="ml-auto">
                    {renderBadge(tab.badge, 'bg-red-500')}
                  </div>
                )}
              </button>
            ))}
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
              <p className={`text-xs font-semibold uppercase tracking-wide mb-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>AI Tools</p>
              <div className="grid grid-cols-2 gap-2">
                {aiTools.slice(0, 6).map((tool, index) => (
                  <button
                    key={index}
                    onClick={() => handleAIToolClick(tool.tool)}
                    className={`flex items-center space-x-2 p-2 rounded-lg transition-all duration-200 ${isDark ? 'hover:bg-white/5 text-gray-300 hover:text-white' : 'hover:bg-gray-50 text-gray-600 hover:text-gray-900'}`}
                  >
                    <tool.icon size={14} />
