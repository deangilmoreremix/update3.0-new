      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-white">Domain Health Monitor</h2>
          </div>
          <div className="flex items-center space-x-4">
            <span className="text-white/70 text-sm">Last updated: {lastUpdate}</span>
            <button
        ...
        {overallStatus && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white/10 rounded-lg p-4 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{overallStatus.total}</p>
                </div>
            ...
            <div className="bg-white/10 rounded-lg p-4 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-400">{overallStatus.up}</p>
                </div>
            ...
            <div className="bg-white/10 rounded-lg p-4 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white/70 text-sm">Avg. Uptime</p>
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        ...
        <div className="space-y-4">
          {healthData.map(domain => (
            <div key={domain.id} className="bg-white/10 rounded-lg p-4 border border-white/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-white/70 text-sm">DNS:</span>
                  <span className={`text-sm ${domain.dnsStatus === 'healthy' ? 'text-green-400' : 'text-red-400'}`}>
                    {domain.dnsStatus}
                  </span>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-white/70 text-sm">SSL:</span>
                  <span className={`text-sm ${domain.sslStatus === 'valid' ? 'text-green-400' : 'text-yellow-400'}`}>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-white text-sm">{domain.location}</span>
                </div>
              </div>
              ...
              {domain.incidents.length > 0 && (
                <div className="mt-4 pt-4 border-t border-white/20">
                  <h4 className="text-white/70 text-sm mb-2">Recent Incidents</h4>
                    ...
                    {domain.incidents.slice(0, 3).map(incident => (
                      <div key={incident.id} className="flex items-center justify-between text-sm">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-white/70">{incident.type.replace('_', ' ')}</span>
                          <span className="text-white">
                            {new Date(incident.timestamp).toLocaleDateString()}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <span className="text-white/70">{incident.duration}min</span>
                          {incident.resolved && (
