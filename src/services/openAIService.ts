...
class OpenAIService {
   ...
   */
  async generateDealInsights(dealData: any, customerId?: string, model?: string): Promise<unknown> {
    if (!this.isValidApiKey()) {
   ...
   */
  async analyzePipelineHealth(pipelineData: any, customerId?: string, model?: string): Promise<unknown> {
    if (!this.isValidApiKey()) {
